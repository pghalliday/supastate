...
const usersIdNotNullConstraint = create.addNotNullConstraint({column: usersIdColumn, external: true});
...
const publicT1IdNotNullConstraint = create.addNotNullConstraint({column: publicT1IdColumn});
...
const s1T1IdNotNullConstraint = create.addNotNullConstraint({column: s1T1IdColumn});
supatest.setSupastate(create);

supatest.colNotNull(usersIdColumn);
supatest.colNotNull(publicT1IdColumn);
supatest.colNotNull(s1T1IdColumn);
