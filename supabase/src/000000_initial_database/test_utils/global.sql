...
create function tu_has_rls(schemaname name, tablename name) returns text as $_func_$
    select results_eq(
        format(
            $$
                select rowsecurity
...
create function tu_has_security_invoker(schemaname name, viewname name) returns text as $_func_$
    select results_eq(
        format($$
            select
